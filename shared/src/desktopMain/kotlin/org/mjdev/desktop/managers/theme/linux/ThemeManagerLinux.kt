...
class ThemeManagerLinux(
    ...
    private val palette = context.palette
    private val themeName = THEME_MJDEV
    private val homeDir = context.currentUser?.homeDir ?: "/".toPath()
...
//            Log.i("Deleting theme : file:///${themeDesktopFile.parentFile.absolutePath}")
    ...
    override fun createFromPalette() {
        runCatching {
            ...
            CompositorControl.setDecorationTheme(
                palette.backgroundColor,
                palette.textColor,
                palette.iconsTintColor,
                palette.borderColor,
            )
            applyLiveGtkTheme()
        }.onFailure { e ->
    ...
    /**
     * Push palette colors into already-running GTK apps. Writing gtk.css alone is not enough —
     * GTK loads the theme at startup and caches it. Toggling [gtk-theme] over gsettings emits the
     * settings notify that open apps listen for and reloads our updated Mjdev theme files.
     */
    ...
    companion object {
        ...
        private fun GtkTheme.write() {
            file.apply {
                ...
                delete()
                val data = asString()
                Log.i("Writing theme file: file://${file.absolutePath}")
                writeText(data)
            }
        }
    }
