...
class KAVFormatContext {
    ...
    fun findVideoStreams(): List<KVideoStream> = (0 until fmtCtx.nb_streams()).mapNotNull { i ->
        ...
        if (type == KAVMediaType.VIDEO) {
            ...
            val duration =
                stream.duration() * 1000L * stream.time_base().num().toLong() / stream.time_base().den().toLong()
            val fps = stream.avg_frame_rate().num().toFloat() / stream.avg_frame_rate().den().toFloat()
            val bitrate = stream.codecpar().bit_rate()
            val codecName =
                avcodec.avcodec_get_name(stream.codecpar().codec_id()).getString(Charset.defaultCharset())
            KVideoStream(
                id = i,
                width = width,
                height = height,
    ...
    private fun findHWDecoder(codecId: Int): List<KHWDecoder> {
        val codec = avcodec.avcodec_find_decoder(codecId) ?: throw IllegalStateException("No codec found!")
    ...
    init {
        ...
        if (hwDecoder != null) {
            ...
            errCheck(
                ...
                "Unable to create hw decoder ${hwDecoder.type}:${hwDecoder.name} for ${
                    codec.name().getString(Charset.defaultCharset())
                }"
            )
