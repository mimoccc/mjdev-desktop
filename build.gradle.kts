...
tasks.register("packageMjdevDeb") {
    ...
    dependsOn("createDistributable", ":compositor:linkMjdevcReleaseExecutableLinuxX64")
    doLast {
        val appName = libs.versions.appName.get()
        val version = libs.versions.packageVersion.get()
        val appDescription = libs.versions.appDescription.get()
        // catalog format is "email<Name>", debian control wants "Name <email>"
        val maintainer = Regex("(.+)<(.+)>").find(libs.versions.appMaintainer.get())
            ?.let { "${it.groupValues[2].trim()} <${it.groupValues[1].trim()}>" }
            ?: libs.versions.appMaintainer.get()
...
// portable app image archive ("apk") next to the deb in release/
