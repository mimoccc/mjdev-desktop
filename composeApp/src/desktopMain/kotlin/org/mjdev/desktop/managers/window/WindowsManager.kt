...
class WindowsManager(
    ...
    override val isCompositorAvailable: Boolean
        get() = socketPath != null

    private val windowsState = mutableStateOf<List<SystemWindow>>(emptyList())
    private var eventsJob: Job? = null

    override val windows: List<SystemWindow>
    ...
    override fun findWindows(app: IApp?): List<SystemWindow> =
        if (app == null) windows else windows.filter { it.matches(app) }

    override fun hasWindow(app: IApp): Boolean =
        findWindows(app).isNotEmpty()

    override fun isWindowFocus(app: IApp): Boolean =
        findWindows(app).any { it.focused }

    override fun requestWindowsFocus(app: IApp): Boolean =
        commandForAll(app, "activate")

    override fun minimizeWindows(app: IApp): Boolean =
        commandForAll(app, "minimize")

    override fun closeWindows(app: IApp): Boolean =
    ...
    private fun SystemWindow.matches(app: IApp): Boolean =
    ...
    private fun commandForAll(app: IApp, cmd: String): Boolean {
